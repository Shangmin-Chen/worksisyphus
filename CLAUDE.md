...
4. **Apply (1-step compile, ATS check, freeze & SQLite sync):**
   ...
   This creates `applications/<YYYY-MM-DD>_<app-stem>/` with `jd.txt` (verbatim posting), `plan.json`, `Simon_Chen_Resume.pdf`, and `meta.json` (`company`, `role`, `date`, `source_url`, `status: "applied"`, `evaluation` — the HackerRank hiring-agent score for the resume as sent — and `contact_verification`, recording whether the contact block was cross-checked against the database and, if not, why; folders published before that field existed simply omit it, which reads as "not recorded" — and `trimmed`, recording each trim-loop cut by kind and slug, with bullet trims also naming the removed bullet; folders published before that field existed simply omit it, which reads as "not recorded"), runs the ATS extraction check, inserts into `worksisyphus.db` (including `evaluation_json`), and records the audit event. The resume is compiled directly into that folder and written nowhere else — `applications/` is the only place a delivered resume exists on disk.
...
uv run worksisyphus db sync                      # load profile.json into SQLite
...
Hexagonal Architecture: `core/` (`domain/` models/rules/gates, `use_cases/` application/pipeline/optimizer/evaluator, `rendering/` Jake's LaTeX) is isolated from I/O through `ports/` (`CompilerPort`, `StoragePort`, `AtsExtractorPort`, `RubricsPort`). `adapters/` implement these ports (`inbound/cli/` commands, `outbound/latex/`, `persistence/` SQLite, `pdf/` ATS, `filesystem/`). Root modules (`profile.py`, `pipeline.py`, `application.py`, `db.py`, `cli.py`, etc.) serve as backward-compatibility facades. Nothing in the render path reads from SQLite (`profile.json` is the sole source of truth for rendering). Tests use a fixture profile, in-memory SQLite, and an injectable fake compiler; they must keep passing without network or pdflatex.
