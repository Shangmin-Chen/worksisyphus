...
6. The application is automatically recorded in the filesystem (`applications/<date>_<company>_<role>/`) with full metadata, JD text, and the evaluation score for the resume as sent.
...
```bash
uv run worksisyphus apply --company <company> --jd <file|-> [--role <role>] [--url <url>] [--plan <file|->] # 1-step compile, validate, and freeze
...
│   ├── ports/              # abstract interfaces (compiler, ATS)
│   ├── adapters/
│   │   ├── inbound/cli/    # CLI entrypoints, status, argument parsing
│   │   └── outbound/       # latex/pdflatex, pdf/ats, filesystem
...
`applications/<date>_<stem>/plan.json` alongside the compiled PDF and metadata. For `tailor` previews, the plan's
