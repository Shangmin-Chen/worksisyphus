...
from ..filesystem.profile_loader import load_profile
...
def list_applications_from_db(conn: sqlite3.Connection) -> list[dict[str, str]]:
    ...
    from ....core.use_cases.application import sorted_application_names
...
def update_application_status_in_db(
    ...
    from ....core.use_cases.application import match_application_identifier
...
class SqliteTursoAdapter:
    ...
    def list_applications(self) -> list[dict[str, str]]:
        conn = get_connection(self.db_path)
        try:
            return list_applications_from_db(conn)
        finally:
            conn.close()
    ...
    def update_application_status(self, app_id: str, new_status: str) -> None:
        conn = get_connection(self.db_path)
        try:
            update_application_status_in_db(conn, app_id, new_status)
        finally:
            conn.close()
